      ...
      preConfirm: () => {
        {
          // change url
          // to mark present
          return fetch('http://localhost:8080/interviewer/addremarks/'+slot.data_Id,{
            method:"POST",
            headers: {
  ...
  cancelSlot(slot: any) {
    Swal.fire({
      ...
      preConfirm: (login) => {
        ...
        return fetch('http://localhost:8080'+'/interviewer/cancelSlot/'+this.interviewerID+"/"+slot.slot_id.slot_Id,{
          method:'POST',
          headers: {
            'Content-Type': 'application/json'
            // 'Content-Type': 'application/x-www-form-urlencoded',
